...
describe("Basic marketplace Tests", function () {
    ...
    it("Test basic loan and repay", async function () {
        ...
        const {paymentToken, pool, tokenId} = await initiateDefaultSimpleLoan(borrower1, lender1, marketplace, NFT1);
        ...
        await paymentToken.connect(borrower1).approve(await pool.getAddress(), ethers.parseEther("10"));
        ...
        const defaultRepayAmount = ethers.parseEther("110");
        expect(balance).to.eq(defaultRepayAmount);
        expect((await marketplace.loans(loanId)).status).to.eq(1);
        await pool.connect(lender1).withdraw(defaultRepayAmount);
        const lenderBalance = await pool.balance(lender1.address);
        expect(lenderBalance).to.eq(0);
        expect((await paymentToken.balanceOf(lender1.address))).to.eq(defaultRepayAmount);
    });
    ...
    it("Test basic loan and default", async function () {
        ...
        const {pool, tokenId} = await initiateDefaultSimpleLoan(borrower1, lender1, marketplace, NFT1);
        // LTV default loan
        ...
        await marketplace.connect(lender1).liquidate(loanId);
        const NFTOwner = await NFT1.ownerOf(tokenId);
        expect(NFTOwner).to.eq(lender1.address);
        const balance = await pool.balance(borrower1.address);
        expect(balance).to.eq(ethers.parseEther("100"));
        expect((await marketplace.loans(loanId)).status).to.eq(3);
    });

    it("Test basic loan and default for LTV", async function () {
        const {marketplace, lender1, borrower1, NFT1, oracle} = await loadFixture(basicSetup);
        await initiateDefaultSimpleLoan(borrower1, lender1, marketplace, NFT1);
        // LTV default loan
...
async function initiateSimpleLoan(
    ...
    await paymentToken.mint(lender.address, ethers.parseEther("100"));
...
async function initiateLoan(
    ...
    await marketplace.connect(borrower).acceptOffer(offer);
